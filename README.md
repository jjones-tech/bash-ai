...
*   `gemini/` (Root directory):
    ...
    *   `execute_tool.sh`: A general script for executing tools within the Gemini environment.
...
The `execute_tool.sh` script is likely the primary entry point for running the individual tools.  Consult the individual tool directories within `tools/` for specific usage instructions.